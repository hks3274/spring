  	    ...
  	    $("#inputImgs").on("change", function(e) {
  	        ...
  	        filesArr.forEach(function(f) {
  	            if (!f.type.match("image.*")) {
  	                alert("확장자는 이미지 파일만 업로드하실 수 있습니다.");
  	                return false;
  	            }
  	            imgFiles.push(f); 
...
<div class="container">
  ...
  <form name="myform" method="post" enctype="multipart/form-data">
    <a href="javascript:imgUpload()" class="myBtn">이미지 불러오기</a>	
    <input type="file" name="fName" id="inputImgs" style="display: none" accept=".jpg,.gif,.png,.zip,.ppt,.pptx,.hwp" multiple/>  
    <p class="mt-3">
