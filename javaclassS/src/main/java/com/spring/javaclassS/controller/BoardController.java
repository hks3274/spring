...
public class BoardController {
	...
	//1.부모댓글에 대한 입력처리(원본글에 대한 입력처리)
	@ResponseBody
	@RequestMapping(value = "/boardReplyInput", method = RequestMethod.POST)
	public String boardReplyInputPost(BoardReplyVO replyVO) {
		//부모 댓글의 경우는 re_step=0, re_order=1로 처리(단, 원본글의 첫번째 부모댓글은 re_order=1이지만, 2번이상은 마지막 부모댓글의 re_order보다 +1처리해준다.)
	...
	//2.대댓글 입력처리(부모댓글에 대한 입력처리)
	@ResponseBody
	@RequestMapping(value = "/boardReplyInputRe", method = RequestMethod.POST)
	public String boardReplyInputRePost(BoardReplyVO replyVO) {
		//1.대댓글의 부모 댓글의 re_step은 부모댓글의 re_step+1, 2.re_order는 부모의 re_order보다 큰 댓글은 모두 +1 처리 후, 3.자신의 re_order을 +1처리해준다.
	...
	public String boardSearchGet (String search, Model model,
		...
		else if(pageVO.getSearch().equals("nickName")) searchTitle = "글내용";
