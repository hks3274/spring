...
//	}
	...
	public String getPwdChcek(Model model, String pwd) {
		
		//비밀번호 암호화 처리(Spring Security에서 제공해주는 BCryptPasswordEncoder객체를 활용한 암호화)
	...
	public String postPwdCheckOk(String pwd, String encPwd, Model model) {
		
		String pwdFlag = "";
		
//		String imsiPwd = passwordEncoder.encode(pwd); 
//		System.out.println("원본비번 : "+encPwd);
//		System.out.println("사용자비번 : "+imsiPwd);
		
//		if(imsiPwd.equals(encPwd)) {
		
		if(passwordEncoder.matches(pwd, encPwd)) { //matches(원본비밀번호, 암호화비밀번호)
			pwdFlag = "비밀번호가 정확합니다.";
		}
		else pwdFlag = "비밀번호가 틀립니다.";
